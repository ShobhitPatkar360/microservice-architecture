...
app.get('/test2', async (req, res) => {
    try {
        ...
        console.error('Error making request to localhost:3002:', error.message);
        // Handle errors and respond with an error message
        res.status(500).json({
            message: 'Failed to fetch data from localhost:3002',
            error: error.message,
        });
    }
});
...
app.get('/test1', async (req, res) => {
  try {
      // Make a GET request to localhost:3002
      const response = await axios.get("http://3.85.241.165:3001/test1");
      // Send the response from the external API to the client
      res.json({
...
//   axios.get("3.85.241.165:3002/test2/test2")
//   .then(response => {
//     res.send(response.data);
//   })
//   .catch(error => {
